...
% Generates the HH version for KS testing. Does the test with a random test
% of the same size (bootstrapping). Runs the test 20 times and tests at 5%
% confidence interval
...
function matrix = pcolorgen(filepath)
    ...
    for i = 1:50
        ...
        %must have minimum 100 non-NaN elements & be less than 70% of all data
        %(i.e. thresholding)
        if sum(~isnan(mod_masked)) >= 100 && numel(mod_masked) <= 103223*0.7
            mod_masked = mod_masked(~isnan(mod_masked));
            temphval = zeros(1,20);
            for k = 1:20
                sample_ind = randi([1 103223], numel(mod_masked), 1);
                sample_taub = taub(sample_ind);
                
                [temphval(1,k),~] = kstest2(mod_masked, sample_taub, 'Alpha', 0.05);
            end
            if sum(temphval) > 1
                mean_m(j,i+1) = 1;
            else
